...
Camera::Camera(GLfloat x,GLfloat y,GLfloat z, GLfloat rot)
{

	Position = Vector3dCreate (x, y,z); // edit start pos of camera
	View = Vector3dCreate( 0.0, 0.0, -1); //previous View = Vector3dCreate( 0.0, 0.0, -1);
	RightVector = Vector3dCreate (1.0, 0.0, 0.0);
	Up = Vector3dCreate (0.0, 1.0, 0.0);
	RotateY(rot);


	RotatedX = RotatedZ = 0.0;
}
