...
void Key(bool* keys, float speed)
{
	...
	if (keys['A'])
		MyCamera.RotateY(2 * speed);
	if (keys[VK_UP] && g.isOut(MyCamera))
		MyCamera.MoveForward(1 * speed);
	if (keys[VK_DOWN] )
		MyCamera.MoveForward(-1 * speed);
	if (keys[VK_RIGHT] && g.isOut(MyCamera))
		MyCamera.MoveRight(1 * speed);
	if (keys[VK_LEFT] && g.isOut(MyCamera))
		MyCamera.MoveRight(-1 * speed);
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	cout<<"x = "<<MyCamera.Position.x;
	cout<<"    y = "<<MyCamera.Position.y;
	cout<<"    z = "<<MyCamera.Position.z<<endl;
	MyCamera.Render();
...
int main(HINSTANCE hInstance, HINSTANCE hPrevInstace, LPSTR IpCmdLine, int nCmdShdow){

	return WinMain(hInstance, hPrevInstace,IpCmdLine,nCmdShdow);
}
